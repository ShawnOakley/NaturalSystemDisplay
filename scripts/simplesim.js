...
(function(exports) {
   ...
   */
    ...
    exports.Utils._addEvent(window, 'devicemotion', function(e) {
      ...
      if (window.orientation === 0) {
        world.gravity1.x = x;
        world.gravity1.y = y * -1;
        world.gravity2.x = x;
        world.gravity2.y = y * -1;
        // world.gravity3.x = x;
        // world.gravity3.y = y * -1;
        ...
        world.gravity1.x = y * -1;
        world.gravity1.y = x * -1;
        world.gravity2.x = y * -1;
        world.gravity2.y = x * -1;
        // world.gravity3.x = y * -1;
        // world.gravity3.y = x * -1;
      }
    });
  ...
  System.collisionDetector = function(){
  	var objects = System._records.list.slice(1);
  	for (i=0; i < objects.length; i++) {
  		var objects2 = objects.splice(i, 1);
  		for (j=0; j < objects2.length; j++) {
  			objects[i].overlap(objects2[j])
  		}
  	}
  }
...
(function(exports) {
   ...
   */
    ...
    this.angle = 0;
    this.gravity1 = new exports.Vector(0, -10.5);
    this.gravity2 = new exports.Vector(10.5, 0);
    // this.gravity3 = new exports.Vector(0, 10.5);
    // this.gravity4 = new exports.Vector(-10.5, 0);
    // this.gravity1 = new exports.Vector(this.width/3, this.height/3)
    // this.gravity2 = new exports.Vector(2 * this.width/3, 2 * this.height/3)
    // this.gravity3 = new exports.Vector(2 * this.width/3, this.height/3)
    // this.gravity4 = new exports.Vector(this.width/3, 2 * this.height/3)
  ...
  // Returns truthy value if given space is occupied, otherwise returns false
...
(function(exports) {
  ...
  Item.prototype.init = function(opt_options) {
    ...
    this.bounciness = options.bounciness || 1.00;
  ...
  Item.prototype.step = function() {
    ...
    this.applyForce(this.world.gravity1);
    this.applyForce(this.world.gravity2);
    // this.applyForce(this.world.gravity3);
  ...
  // Checks for overlap between a given object and this object

  Item.prototype.overlap = function(item){
  	var checkRight = item.location.x + item.width/2;
  	var checkLeft = item.location.x - item.width/2;
  	var checkUp = item.location.y - item.width/2;
  	var checkDown = item.location.y + item.width/2;

  	// See here for proper modeling techinique: http://www.wildbunny.co.uk/blog/2011/04/20/collision-detection-for-dummies/

	    if ((this.location.x + this.width/2 >= checkLeft) && ( checkLeft > this.location.x - this.width/2)) {
	      this.velocity.x *= -1 * this.bounciness;
	      item.velocity.x *= -1 * item.bounciness;
	    } 

	   	if ((this.location.x - this.width/2 <= checkRight) && ( checkRight > this.location.x - this.width/2)) {
	      this.velocity.x *= -1 * this.bounciness;
	      item.velocity.x *= -1 * item.bounciness;
	    } 

	   	if ((this.location.y + this.height/2 >= checkUp) && ( checkUp < this.location.y - this.height/2)) {
	      this.velocity.y *= -1 * this.bounciness;
	      item.velocity.y *= -1 * item.bounciness;
	    } 

	   	if ((this.location.y + this.height/2 <= checkDown) && ( checkLeft > this.location.y - this.height/2)) {
	      this.velocity.y *= -1 * this.bounciness;
	      item.velocity.y *= -1 * item.bounciness;
	    } 
  ...
  Item.prototype.distance = function(item) {
  	Math.sqrt(Math.pow((this.location.x - item.location.x), 2) + Math.pow((this.location.y - item.location.y),2))
  }
